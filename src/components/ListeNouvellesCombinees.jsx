...
// Cache simple en mémoire pour les nouvelles UdeM
const newsCache = {
  data: null,
  timestamp: null,
...
export default function ListeNouvellesCombinees({ 
  ...
  // Récupération des nouvelles locales via GraphQL
  ...
  useEffect(() => {
    ...
    const fetchAndCombineNews = async () => {
      setLoading(true)
      setError(null)
      
      try {
        // 1. Préparer les nouvelles locales
        const imagesMap = new Map()
        images.nodes.forEach((node) => {
          imagesMap.set(node.name, node.childrenImageSharp[0]?.gatsbyImageData)
        })

        const localNews = allFile.nodes.map((node) => {
            const { id, relativePath } = node
            const {
                excerpt,
                frontmatter: { date, newsImage, slug, title, type, newsUrl },
            } = node.childMdx
            ...
            const link = type === 'interne' 
                ? `/nouvelles/${year}/${slug}`  // Ajout de l'année dans l'URL
                : newsUrl || '#'
            
            return {
                id,
                type: type === 'interne' ? 'interne' : 'externe',
                title,
                excerpt,
                date,
                formattedDate: date,
                ...
                link,
                image: newsImage?.name ? imagesMap.get(newsImage.name) : null,
                imageAlt: newsImage?.alt || '',
                year  // On conserve l'année au cas où
            }
            })

        // 2. Récupérer les nouvelles UdeM
        let udeMNews = []
        if (newsCache.isValid) {
          ...
          udeMNews = (await Promise.all(feedPromises)).flat()
          newsCache.data = udeMNews
          newsCache.timestamp = Date.now()
        }

        // 3. Combiner et trier toutes les nouvelles
        const allNews = [...localNews, ...udeMNews]
  ...
  }, [allFile.nodes, images.nodes, rssFeeds])
    ...
    >
      ...
      <Box sx={{ textAlign: 'center', mt: 2 }}>
        <Button
          ...
          endIcon={
            <span className="MuiButton-icon MuiButton-endIcon MuiButton-iconSizeMedium css-1g78ho2">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="#616161" viewBox="0 0 256 256">
                <path d="M200,64V168a8,8,0,0,1-16,0V83.31L69.66,197.66a8,8,0,0,1-11.32-11.32L172.69,72H88a8,8,0,0,1,0-16H192A8,8,0,0,1,200,64Z"></path>
              </svg>
            </span>
          }
